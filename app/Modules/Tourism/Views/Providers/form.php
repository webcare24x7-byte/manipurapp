<?php declare(strict_types=1);$d=$record??$old??[];$loc=$locations??[];$base=config('app.base_path');?><div class="card"><div class="form-grid"><div class="form-group"><label>Business Name *</label><input name="business_name" required value="<?= htmlspecialchars((string)($d['business_name']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group"><label>Provider Type *</label><select name="provider_type" required><?php foreach(['HOTEL'=>'Hotel','LODGE'=>'Lodge','HOMESTAY'=>'Homestay','RESORT'=>'Resort','GUEST_HOUSE'=>'Guest House','TOUR_OPERATOR'=>'Tour Operator','TRAVEL_AGENCY'=>'Travel Agency','OTHER'=>'Other'] as $k=>$v): ?><option value="<?= $k ?>" <?= ($d['provider_type']??'OTHER')===$k?'selected':'' ?>><?= $v ?></option><?php endforeach; ?></select></div><div class="form-group"><label>Legal / Owner Name</label><input name="legal_name" value="<?= htmlspecialchars((string)($d['legal_name']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group"><label>Phone</label><input name="phone" value="<?= htmlspecialchars((string)($d['phone']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group"><label>Email</label><input type="email" name="email" value="<?= htmlspecialchars((string)($d['email']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group"><label>Postal Code</label><input name="postal_code" value="<?= htmlspecialchars((string)($d['postal_code']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group full-width"><label>Address</label><input name="address" value="<?= htmlspecialchars((string)($d['address']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group"><label>City / Town</label><select name="city"><option value="">Select...</option><?php foreach($loc['cities']??[] as $v): ?><option <?= ($d['city']??'')===$v?'selected':'' ?>><?= htmlspecialchars($v,ENT_QUOTES,'UTF-8') ?></option><?php endforeach; ?></select></div><div class="form-group"><label>District</label><select name="district"><option value="">Select...</option><?php foreach($loc['districts']??[] as $v): ?><option <?= ($d['district']??'')===$v?'selected':'' ?>><?= htmlspecialchars($v,ENT_QUOTES,'UTF-8') ?></option><?php endforeach; ?></select></div><div class="form-group"><label>State</label><select name="state"><?php foreach($loc['state']??['Manipur'] as $v): ?><option <?= ($d['state']??'Manipur')===$v?'selected':'' ?>><?= htmlspecialchars($v,ENT_QUOTES,'UTF-8') ?></option><?php endforeach; ?></select></div><div class="form-group"><label>Status</label><select name="status"><?php foreach(['Active','Inactive','Suspended'] as $v): ?><option <?= ($d['status']??'Active')===$v?'selected':'' ?>><?= $v ?></option><?php endforeach; ?></select></div><div class="form-group full-width"><label>Description</label><textarea name="description" rows="4"><?= htmlspecialchars((string)($d['description']??''),ENT_QUOTES,'UTF-8') ?></textarea></div></div></div><div class="card location-card"><h2>Provider Location</h2><p>Click anywhere on the map to place the marker, drag the marker to fine-tune it, or use your current location.</p><div class="map-picker" id="tour-provider-map"></div><div class="form-grid location-fields"><div class="form-group"><label>Latitude</label><input id="tour-provider-lat" type="number" step="0.0000001" min="-90" max="90" name="latitude" value="<?= htmlspecialchars((string)($d['latitude']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group"><label>Longitude</label><input id="tour-provider-lng" type="number" step="0.0000001" min="-180" max="180" name="longitude" value="<?= htmlspecialchars((string)($d['longitude']??''),ENT_QUOTES,'UTF-8') ?>"></div><div class="form-group full-width"><button type="button" class="btn btn-secondary" id="tour-provider-current">Use Current Location</button> <button type="button" class="btn btn-secondary" id="tour-provider-clear">Clear</button><span id="tour-provider-status"></span></div></div></div><style>.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.full-width{grid-column:1/-1}.form-group label{display:block;font-weight:600;margin-bottom:7px}.form-group input,.form-group select,.form-group textarea{width:100%;box-sizing:border-box}.location-card{border:1px solid #dbeafe}.location-card p,#tour-provider-status,#dest-status,#stay-status{color:#64748b;font-size:13px}.map-picker{height:360px;border-radius:14px;overflow:hidden;border:1px solid #cbd5e1;margin:14px 0}.location-fields{margin-top:4px}.form-group small{display:block;color:#64748b;margin-top:6px}@media(max-width:700px){.form-grid{grid-template-columns:1fr}.full-width{grid-column:auto}.map-picker{height:300px}}</style><link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>(function(){
const lat=document.getElementById('tour-provider-lat'),lng=document.getElementById('tour-provider-lng'),status=document.getElementById('tour-provider-status');
const mapEl=document.getElementById('tour-provider-map'); if(!lat||!lng||!mapEl)return;
...
function init(){
  ...
  try{
    ...
    document.getElementById('tour-provider-current')?.addEventListener('click',()=>{
    ...
    document.getElementById('tour-provider-clear')?.addEventListener('click',()=>{
