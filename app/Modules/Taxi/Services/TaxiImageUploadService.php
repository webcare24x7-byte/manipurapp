...
final class TaxiImageUploadService
{
    ...
    /**
     * Store one public-facing Taxi image and return its relative public path.
     */
    public function store(array $file, string $category): string
    {
        if (!in_array($category, ['vehicles', 'drivers'], true)) {
            throw new RuntimeException('Invalid taxi image category.');
        }
        ...
        $relativeDirectory = 'uploads/taxi/' . $category;
        $directory = $root . '/public/' . $relativeDirectory;

        if (!is_dir($directory) && !mkdir($directory, 0755, true) && !is_dir($directory)) {
            throw new RuntimeException('Unable to create the Taxi image upload directory.');
        }
